...
FUNC_RESTORE_DB(){
    ...
    sudo su postgres -c "export PGPASSFILE="$DB_BACKUP_PATH/.pgpass"; pg_restore -d $DB_NAME -f $RESTORE_FILE_SQL"
    #sudo su postgres -c "export PGPASSFILE="$DB_BACKUP_PATH/.pgpass"; pg_restore -U ß -f $DB_NAME -f $RESTORE_FILE_SQL"
